...
const TOOLS = {
  ...
  gov_live_mcp_policy: {
    description: "Read the production-MCP operation-class policy and receipt roots.",
    inputSchema: {
      type: "object",
      properties: {
        json: { type: "boolean" },
    ...
    handler: ({ json }) => runGov(json ? ["live-mcp-policy", "--json"] : ["live-mcp-policy"], { captureJson: Boolean(json) }),
  },

  gov_live_mcp_record: {
    description: "Record a governed production-MCP receipt. Mutating: writes a local receipt artifact; real production calls stay adapter-owned.",
    inputSchema: {
      type: "object",
      properties: {
        ticket: { type: "string" },
        operationClass: { type: "string", enum: ["read_safe", "read_sensitive", "write_low", "write_prod", "destructive"] },
        adapter: { type: "string" },
        ...
        result: { type: "string", enum: ["pass", "fail", "observed", "blocked"] },
      },
      required: ["ticket", "operationClass", "adapter", "operation", "scope", "evidence"],
    },
    handler: ({ ticket, operationClass, adapter, operation, scope, redaction, approval, cleanup, evidence, result }) => {
      const args = ["live-mcp-record", ticket, "--class", operationClass, "--adapter", adapter, "--operation", operation, "--scope", scope];
      if (redaction) args.push("--redaction", redaction);
      if (approval) args.push("--approval", approval);
      if (cleanup) args.push("--cleanup", cleanup);
      if (result) args.push("--receipt-result", result);
      pushRepeated(args, "--evidence", evidence);
      return runGov(args, { captureJson: false });
    },
  },

  gov_bootstrap_record: {
    description: "Record a server-bootstrap/backfill receipt with idempotency, resource envelope, observability, and rollback evidence.",
    inputSchema: {
      type: "object",
      properties: {
        ticket: { type: "string" },
        ...
        resourceEnvelope: { type: "string" },
        idempotency: { type: "string" },
        queryShape: { type: "string" },
        observability: { type: "string" },
        disableRollback: { type: "string" },
        evidence: { type: "array", items: { type: "string" } },
        result: { type: "string", enum: ["pass", "fail", "observed", "blocked"] },
      },
      required: ["ticket", "job", "executionMode", "resourceEnvelope", "idempotency", "observability", "disableRollback", "evidence"],
    },
    handler: ({ ticket, job, executionMode, resourceEnvelope, idempotency, queryShape, observability, disableRollback, evidence, result }) => {
      const args = [
        "bootstrap-record", ticket,
        "--job", job,
        "--execution-mode", executionMode,
        "--resource-envelope", resourceEnvelope,
        "--idempotency", idempotency,
        "--observability", observability,
        "--disable-rollback", disableRollback,
      ];
      if (queryShape) args.push("--query-shape", queryShape);
      if (result) args.push("--receipt-result", result);
      pushRepeated(args, "--evidence", evidence);
      return runGov(args, { captureJson: false });
    },
  ...
  gov_deploy_record: {
    description: "Record a deployment-lane receipt proving the intended artifact and source were shipped.",
    inputSchema: {
      type: "object",
      properties: {
        ticket: { type: "string" },
        ...
        runningArtifact: { type: "string" },
        deployId: { type: "string" },
        operator: { type: "string" },
        rollback: { type: "string" },
        evidence: { type: "array", items: { type: "string" } },
      },
      required: ["ticket", "environment", "commit", "artifact", "runningArtifact", "deployId", "operator", "rollback"],
    },
    handler: ({ ticket, environment, commit, buildSource, artifact, runningArtifact, deployId, operator, rollback, evidence }) => {
      const args = [
        "deploy-record", ticket,
        "--environment", environment,
        "--commit", commit,
        "--artifact", artifact,
        "--running-artifact", runningArtifact,
        "--deploy-id", deployId,
        "--operator", operator,
        "--rollback", rollback,
      ];
      if (buildSource) args.push("--build-source", buildSource);
      pushRepeated(args, "--evidence", evidence);
      return runGov(args, { captureJson: false });
    },
  },
  ...
  gov_deploy_check: {
    description: "Validate a deployment receipt and assert running artifact identity equals landed source.",
    inputSchema: {
      type: "object",
      properties: {
        ticket: { type: "string" },
    ...
    handler: ({ ticket, receipt, json }) => {
      const args = ["deploy-check"];
      if (ticket) args.push(ticket);
      if (receipt) args.push("--receipt", receipt);
      if (json) args.push("--json");
      return runGov(args, { captureJson: Boolean(json) });
    },
  },

  gov_verify_runtime: {
    description: "Record a post-deploy runtime verification receipt for a ticket claim.",
    inputSchema: {
      type: "object",
      properties: {
        ticket: { type: "string" },
        environment: { type: "string" },
        evidenceClass: { type: "string", enum: ["fixture", "gate", "runtime", "mcp-oracle", "deploy", "bootstrap"] },
        result: { type: "string", enum: ["pass", "fail", "observed", "blocked"] },
        claim: { type: "string" },
        evidence: { type: "array", items: { type: "string" } },
      },
      required: ["ticket", "environment", "evidenceClass", "result", "claim", "evidence"],
    },
    handler: ({ ticket, environment, evidenceClass, result, claim, evidence }) => {
      const args = ["verify", ticket, "--environment", environment, "--evidence-class", evidenceClass, "--receipt-result", result, "--claim", claim];
      pushRepeated(args, "--evidence", evidence);
      return runGov(args, { captureJson: false });
    },
  },
  ...
  gov_falsify: {
    description: "Record that later runtime evidence falsified a prior ticket closure.",
    inputSchema: {
      type: "object",
      properties: {
        ticket: { type: "string" },
      ...
      required: ["ticket", "by", "reason", "evidence"],
    },
    handler: ({ ticket, by, reason, evidence }) => {
      const args = ["falsify", ticket, "--by", by, "--reason", reason];
      pushRepeated(args, "--evidence", evidence);
      return runGov(args, { captureJson: false });
    },
  },
  ...
  gov_validate_receipt: {
    description: "Validate any runtime evidence receipt JSON file without mutating state.",
    inputSchema: {
      type: "object",
      properties: {
        receipt: { type: "string" },
    ...
    handler: ({ receipt, json }) => {
      const args = ["validate-receipt", "--receipt", receipt];
      if (json) args.push("--json");
      return runGov(args, { captureJson: Boolean(json) });
    },
  },
