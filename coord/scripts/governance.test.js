  ...
  try {
    ...
    assert.throws(
      ...
      (error) =>
        ...
        // GCV-2 strengthened: without an origin remote (or when the fetch
        // for the requested base fails), the fail-closed message now
        // names the failed fetch + the freshness invariant — strictly
        // stronger than the prior bare "missing base ref" wording, and
        // still fail-closed (no worktree, no silent stale-base fallback).
        /failed to fetch origin missing-base/.test(error.message) &&
...
test("GCV-2: ensureGitWorktree fails closed when origin fetch fails (no stale tracking-ref fallback)", () => {
  // Reviewer reproduction: a local origin/<base> tracking ref already
  // exists (from a prior successful fetch/push) but the remote URL is
  // broken now, so `git fetch` fails. Without this guard the worktree
  // would silently be cut from the (potentially stale) local tracking ref
  // — exactly the "no stale base" intent GCV-2 must enforce.
  const repo = createTempGitRepoWithOrigin("ebmr-gcv2-fetch-fail-", {
    "README.md": "backend\n",
  }, "seed");
  // Confirm the tracking ref exists locally (it does, set by the
  // push during fixture setup).
  assert.equal(__testing.gitCommitishExists(repo.repoRoot, "origin/dev"), true);
  // Break the remote URL so any subsequent fetch fails.
  runGit(repo.repoRoot, ["remote", "set-url", "origin", "/tmp/does-not-exist-ebmr-gcv2"]);
  // Sanity: a direct fetch now fails but the local tracking ref still
  // resolves (the dangerous state).
  assert.equal(__testing.gitCommitishExists(repo.repoRoot, "origin/dev"), true);
  ...
  try {
    assert.throws(
      ...
      (error) =>
        error instanceof GovernanceError &&
        // The freshness-failure error names the failed fetch and the
        // remote ref it would have based on; it must NOT degrade to a
        // bare "missing base" or silently succeed.
        /failed to fetch origin dev/.test(error.message) &&
        /cannot be proven/.test(error.message)
    );
    // And no worktree directory was created.
    assert.equal(fs.existsSync(worktree), false);
  } finally {
    __testing.paths.REPO_ROOTS = original.REPO_ROOTS;
    __testing.paths.REPO_INTEGRATION_BRANCHES = original.REPO_INTEGRATION_BRANCHES;
