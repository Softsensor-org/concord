...
function stableDetail(finding) {
  switch (finding.check) {
    case "size":
    case "monolith":
      // Identity = the file itself (one size/monolith finding per file).
      return "loc";
    case "complexity":
      // Identity = the function name (parsed out of the message); falls back to
      // the start line so two same-named functions stay distinct.
      {
    ...
    case "imports":
      // Identity = the disallowed import specifier.
      return `imp:${normalizeFilePath(finding.value)}`;
    case "duplication":
      // Identity = the duplicate-region hash embedded in the message (stable as
      // long as the duplicated text is unchanged). Falls back to the line.
      {
        const m = /hash\s+([0-9a-f]+)/.exec(String(finding.message || ""));
        return m ? `dup:${m[1]}` : `dup:line${finding.line || 0}`;
      }
    case "hardcoding":
      // Identity = the literal/threshold marker + the start line so the same
      // leak at the same site round-trips. value is the literal (path), a
      // repeat count, or a number — combine with line for stability.
      return `hc:${normalizeFilePath(String(finding.value))}:line${finding.line || 0}`;
    case "deadcode":
      // Identity = the unreferenced symbol name (the finding value).
...
function stableFindingKey(finding) {
  return `${finding.check}:${normalizeFilePath(finding.file)}:${stableDetail(finding)}`;
}
