...
function describeTicketMutationOwnershipIssue(ticketId, row, lock = null) {
  ...
  try {
    ...
    return {
      ...
      message:
        `No active claimed agent session is bound to the current thread for ${ticketId}. ` +
        `Run \`coord/scripts/gov resume ${ticketId}\`, ` +
        `\`coord/scripts/gov claim --owner ${expectedOwner}\`, or pass \`--owner ${expectedOwner}\` on the lifecycle command.`,
      next_steps: [
  ...
  if (identity.agent.handle !== expectedOwner) {
    ...
    return {
      ...
      message:
        ...
        `(or \`coord/scripts/gov claim ${ticketId} --force\`) first.`,
      next_steps: [
        `coord/scripts/gov resume ${ticketId}`,
        `coord/scripts/gov claim --owner ${expectedOwner}`,
