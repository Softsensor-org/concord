...
module.exports = function createWorktreeOps(deps = {}) {
  ...
  function fetchRepoRef(repoRoot, refName) {
    ...
    const result = gitTry(repoRoot, ["fetch", "origin", remoteRef]);
  ...
  function ensureGitWorktree({ repoCode, worktree, branch, base }) {
    ...
    if (branchExists) {
      ...
      // GCV-2: a new governed branch MUST be cut from the authoritative
      // remote, never a stale local ref. Fetch and base on origin/<baseRef>.
      // No local-branch fallback path (the old `baseRef` path produced
      // stale bases when local `dev` lagged origin/dev — FE-385/FE-386).
      //
      // Freshness rule: if the fetch FAILS, we cannot prove origin/<baseRef>
      // is current — a pre-existing local origin/<baseRef> tracking ref may
      // itself be stale (e.g. broken remote URL, network down, auth lost).
      // Fail closed BEFORE the existence check so a stale tracking ref is
      // never silently used as the base.
      const remoteBaseRef = `origin/${String(baseRef).replace(/^origin\//, "")}`;
      const fetched = fetchRepoRef(repoRoot, baseRef);
      if (!fetched) {
        fail(
          `Cannot create governed ${repoBootstrapLabel(repoCode)} worktree: ` +
          `failed to fetch origin ${baseRef} in ${repoRoot}, and freshness of ` +
          `${remoteBaseRef} cannot be proven. Refusing to cut from a ` +
          `potentially-stale base ref (a local ${remoteBaseRef} tracking ref ` +
          `may already exist but be behind the actual remote). Restore ` +
          `network/remote access (\`git -C ${repoRoot} fetch origin ${baseRef}\`) ` +
          `and rerun the same gov start command.`
        );
      }
      if (!gitCommitishExists(repoRoot, remoteBaseRef)) {
        fail(formatMissingStartBaseRefMessage(repoCode, repoRoot, remoteBaseRef));
      }
      runGit(repoRoot, ["worktree", "add", worktree, "-b", branch, remoteBaseRef]);
    }
    return {
