...
~25-agent parallel governed wave (see that deployment's
`coord/docs/PARALLEL_LANDING_LEARNINGS.md`).
...
`options.base || REPO_INTEGRATION_BRANCHES[<repoVar>] || "dev"`. Sites (downstream analogues —
...
verb for cross-repo umbrellas. Exact code (verified in a downstream proving ground):
