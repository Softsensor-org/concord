...
npm install
npm run demo        # → http://localhost:3002
```

A live cockpit showing a real board: evidenced tickets, review cycles, gate results, and the event timeline. Read the [demo walkthrough](DEMO.md) for what to look at.
...
wires `npm run gov` / `npm run concord`. Upgrade later with `npm run gov -- upgrade`.
